...
MODEL_PATH = os.path.join(MODEL_SAVE_DIR, "align_model.pth")
OUTPUT_PATH = os.path.join(PROJECT_ROOT, "data", "submission_ver2.tsv")

# Diamond Paths
DIAMOND_RAW_PATH = os.path.join(DATA_DIR, "processed2", "diamond_matches.tsv") # Assuming same location as ver1
...
def get_propagation_steps(obo_path, term_to_idx):
    ...
    try:
        ...
        # Fallback if cycles exist (shouldn't in OBO is_a)
...
def load_and_process_diamond(diamond_file, train_terms_file, test_ids, term_to_idx):
    ...
    # 4. Create Map: TestID -> {LocalIdx: Score}
    diamond_preds = merged.groupby(["TestID", "GoID"])["Score"].max().reset_index()

    diamond_scores_map = {}
    for row in tqdm(
        diamond_preds.itertuples(index=False), total=len(diamond_preds), desc="Mapping Diamond"
    ):
...
def predict(alpha=0.5):
    ...
    # 2. Prepare GO Embeddings Tensor (Generated by label_processing.py)
    ...
    with torch.no_grad():
        for i in tqdm(range(num_batches)):
            ...
            prot_emb = torch.tensor(
                np.stack(batch_df["embedding"].tolist())
            ).float().to(DEVICE)
            ...
            # Propagation
            # Propagate max score from children to parents
