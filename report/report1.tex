...
\author{
    ...
    22022191@vnu.edu.vn, 2202xxxx@vnu.edu.vn, 2202xxxx@vnu.edu.vn}
}
...
\subsection{A. Mã nguồn EDA}
Dưới đây là các giải thuật chính được sử dụng để phân tích dữ liệu, được đánh số theo thứ tự thực hiện.

% --- ALGORITHM 1 ---
\begin{algorithm}
\caption{Tải dữ liệu và Phân tích Độ dài Chuỗi}
\begin{algorithmic}[1]
\REQUIRE \textit{Files:} \texttt{train\_sequences.fasta}, \texttt{testsuperset.fasta}
\ENSURE Histograms, Metrics ($Mean, Median$)

\STATE $S_{train} \leftarrow ParseFasta(TrainPath)$
\STATE $S_{test} \leftarrow ParseFasta(TestPath)$
\STATE $L_{train} \leftarrow [Length(s) \textbf{ for } s \textbf{ in } S_{train}]$

\FOR{dataset \textbf{in} $\{L_{train}, L_{test}\}$}
    \STATE $PlotHistogram(dataset, bins=100)$
    \STATE $DrawLine(Median(dataset), color='red')$
    \STATE $SaveFigure("sequence\_dist.png")$
    \STATE \textbf{Print} $\{Mean, Median, Std, Min, Max\}$
\ENDFOR
\end{algorithmic}
\label{alg:eda_seq}
\end{algorithm}
...
% --- ALGORITHM 2 ---
\begin{algorithm}
\caption{Phân tích Phân phối Nhãn GO và Loài}
\begin{algorithmic}[1]
\REQUIRE \texttt{train\_terms.tsv}, \texttt{train\_taxonomy.tsv}
\ENSURE Charts: Top-K, Long-tail

\STATE \textbf{1. GO Analysis}
\STATE $Counts_{GO} \leftarrow ValueCounts(Terms)$
\STATE $Top30 \leftarrow Head(Counts_{GO}, 30)$
\STATE $PlotBarH(Top30)$ \COMMENT{Top Frequent Terms}
\STATE $PlotLogLog(Range(Counts_{GO}), Counts_{GO})$ \COMMENT{Power Law}

\STATE \textbf{2. Taxonomy Analysis}
\STATE $Counts_{Tax} \leftarrow ValueCounts(Taxonomy)$
\STATE $Top20 \leftarrow Head(Counts_{Tax}, 20)$
\STATE $PlotBarH(Top20)$ \COMMENT{Dominant Species}
\STATE $PlotHistogram(Counts_{Tax}, yscale='log')$
\end{algorithmic}
\label{alg:eda_dist}
\end{algorithm}
...
% --- ALGORITHM 3 ---
\begin{algorithm}
\caption{Đánh giá Mất cân bằng theo Ontology}
\begin{algorithmic}[1]
\REQUIRE Labels grouped by $\{BPO, CCO, MFO\}$
\ENSURE Gini Index, Imbalance Ratio (IR)

\FOR{ontology $O$ \textbf{in} $\{BPO, CCO, MFO\}$}
    \STATE $Freq \leftarrow ValueCounts(Filter(Terms, O))$
    \STATE $PlotHistogram(Log10(Freq + 1))$
    \STATE $IR \leftarrow Max(Freq) / Min(Freq)$
    \STATE $Gini \leftarrow CalculateGini(Freq)$
    \STATE \textbf{Print} $O, IR, Gini$
\ENDFOR
\end{algorithmic}
\label{alg:eda_imbalance}
\end{algorithm}
\subsection{B. Mã nguồn Tiền xử lý (Data Preprocessing)}
...
\subsection{C. Mã nguồn Huấn luyện và Kiểm thử}
